...
| `GET` | `/api/debates/{id}/` | **Built and verified (spec 0008).** Status, `turn_strategy`, nested `Verdict` if present. Also returns `status_display` (spec 0025, added alongside the raw `status` field) — the human-readable label from `Debate.Status.choices` (e.g. "No consensus" for `NO_CONSENSUS`), for display only; clients must keep branching on the raw `status` value. Now also returns nested `human_review` (spec 0039, built) — `null` if not yet reviewed, else `{id, final_decision, comment, reviewer, reviewed_at}`. |
...
| `POST` | `/api/debates/{id}/review/` | **Built and verified (spec 0039).** Body: `final_decision` (any string from `CaseTypeConfig.decision_options` for this debate's case type — not server-validated against that list today, or `null`/omitted if that list is empty), `comment` (required). Ownership-checked (404 if not owned/nonexistent); 409 if this debate already has a review — the `HumanReview` model's `OneToOneField` on `debate` enforces one-per-debate at the DB level, this endpoint surfaces that as a clean 409 rather than a 500. No separate `GET` — the review is folded into `GET /api/debates/{id}/`'s `human_review` field above (the debate detail page already owns loading this data, no reason for a second round-trip). |

### Config & personas
| Method | Path | Notes |
|---|---|---|
| `GET` | `/api/case-type-configs/` | **Built and verified (spec 0010).** Returns `type` and, as of spec 0039, `decision_options` (the Human Review panel's buttons-vs-comment-only source) — kept minimal otherwise for the case-type picker; `position_options`/`required_fields`/`policy_context` (renamed from `research_guardrail_prompt`, spec 0030) still aren't exposed since no UI needs them yet (not ownership-scoped — shared config). Write access is admin-only (Django admin). |
...
| `POST` | `/api/consultations/{id}/messages` | Body: `{text}`. Delivered via a Temporal Update (`submit_message`) — blocks until the consultant's full reply comes back in the same response: `{message, ready_to_finalize}`. Internally now a 3-node reflection graph (draft → critique → conditional revise, ADR 0008/spec 0023) rather than one call, but this endpoint's request/response contract is completely unchanged by that. For a case type with `CaseTypeConfig.required_fields` defined (`loan_approval` — ADR 0010/spec 0030), `ready_to_finalize` can only become `true` once every required field is actually present in the proposed payload, enforced via a dynamically-built structured-output schema, not just prompt wording; a case type with no required fields (`research_debate`) stays fully free-form, unchanged. 404 if the session isn't owned by the caller; 409 if the session is already approved/failed. |
