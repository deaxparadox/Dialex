...
| Method | Path | Notes |
|---|---|---|
| `GET` | `/api/case-type-configs/` | **Built and verified (spec 0010).** Returns `type` only — kept minimal for the case-type picker; `position_options`/`decision_options`/`required_fields`/`policy_context` (renamed from `research_guardrail_prompt`, spec 0030) aren't exposed here since no UI needs them yet (unlike the plan below, not ownership-scoped — shared config). Write access is admin-only (Django admin). |
