...
The user caught a second real bug immediately after using this milestone: agent sides were inverted and every avatar rendered at the bottom of its bubble instead of the top. Root cause in `.bubble-row`: `flex-direction: row-reverse` was combined with a universal `justify-content: flex-end` and no `.left` override — that combination packs content toward row-reverse's main-*end*, which is the **left** edge, the opposite of intended, so the agent meant to render right was rendering left and vice versa. `align-items: flex-end` was also simply wrong for a multi-line bubble — the avatar should top-align with the header line, not sit at the bottom. Fixed by dropping the row-reverse trick entirely in favor of explicit `order` values on the avatar (unambiguous — no directional footgun to get backwards a second time) and `align-items: flex-start`. Verified with real `getBoundingClientRect()` measurements across 2 rounds on a fresh debate: both agents on consistent, correct, opposite sides; avatar top edge exactly equal to bubble top edge in all 4 measured instances; avatar sitting just outside the bubble's outer edge with no overlap.

## Status

Implemented and verified in a real browser across multiple full debate runs. All items confirmed with concrete evidence: `turn_started`-driven indicators for all 3 stages (opening/argument/verdict) appear with no visible gap and are replaced by real content the instant it lands; avatars, timestamps, and distinct per-agent bubble colors confirmed via `getComputedStyle` (not just visual impression); the opening-statement blank-gap bug and the left/right-inversion/avatar-position bug (found immediately after by the user in real use) both fixed and re-verified clean. 19 Angular tests pass, zero console errors.
